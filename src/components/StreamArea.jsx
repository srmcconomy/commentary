...
export default class StreamArea extends Component {
  ...
  render() {
    ...
    const imageElements = images.map((image, index) => {
      if (index === imageIndex) {
        return <div key={index} className="background current" style={{backgroundImage: `url(/images/${image})`}}></div>
      }
      if (index === imageIndex - 1 || (index === images.length - 1 && imageIndex === 0)) {
        return <div key={index} className="background prev" style={{backgroundImage: `url(/images/${image})`}}></div>
      }
      if (index === imageIndex + 1 || (index === 0 && imageIndex === images.length - 1)) {
        return <div key={index} className="background next" style={{backgroundImage: `url(/images/${image})`}}></div>
      }
      return null;
    })
    return(
